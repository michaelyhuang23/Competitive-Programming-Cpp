...
void dfs(llong cur, llong par){
	...
	for(llong di=0;di<=n;di++){
		llong d = dist2[cur][di];
		llong totalc = 0;
		for(auto& edge : adj[cur]){
			llong son = edge.first;
			llong w = edge.second;
			if(son == par) continue;
			llong llc = inf;
			for(llong pdi=0;pdi<=n;pdi++){
				llong pd = dist2[son][pdi];
				if(pd>d) continue;
				llc = min(llc, dp[son][pdi]+abs(w-(d-pd)));
			}
			totalc += llc;
		}
		dp[cur][di] = min(dp[cur][di], totalc);
	}
}

void get_dist(int src, int cur, int par){
	if(cur==par) dist2[src][cur] = 0;
	for(auto& edge : adj[cur]){
		int son = edge.first;
		int w = edge.second;
		if(son==par)continue;
		dist2[src][son] = dist2[src][cur]+w;
		get_dist(src, son, cur);
	}
}

void solve_brute(){
	adj = vector<vector<pair<llong,llong>>>(n);
	dist2 = vector<vector<llong>>(n,vector<llong>(n+1,0));
	for(llong i=1;i<n;i++){
		llong p, c; cin >> p >> c;
		p--;
		adj[i].pb(mp(p,c));
	...
	dp = vector<vector<llong>>(n, vector<llong>(n+1, inf));
	dfs(0,0);

	llong cost = inf;
	for(llong di=0;di<=n;di++){
		int d = dist2[0][di];
		cost = min(cost, dp[0][di]);
	}
